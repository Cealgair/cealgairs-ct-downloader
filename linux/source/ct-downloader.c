...
#define PATH_TO_WBZ_CONVERTER "path-to-wbz-converter.txt"
...
int main(){
	...
	//clear log
	in_list=fopen(LOG_FILE,"w");
	fprintf(in_list,"No results found:\n");
	fclose(in_list);
	...
	do{
		...
		switch(exit){
			...
			case DL_WBZ:
				...
				break;
			case CONVERT_SZS:
				while(szs_queue->prev!=NULL) szs_queue=szs_queue->prev;
				convert_to_szs(szs_queue->t.name);
				szs_downloaded(&szs_queue,final_track_list);
				break;
			case USER_DECISION:
				//don't wait
			...
			case SLEEP:
				...
				break;
			case DONE:
				printf("\nDone downloading and converting to szs\n");
		}
	}while(exit!=DONE);
	...
	//finish the log
	in_list=fopen(LOG_FILE,"a");
	fprintf(in_list,"\nUnneeded files:\n");
	fclose(in_list);
	print_unneeded_files(final_track_list);
...
BOOL parse_input_file(FILE *in_list,TRACK_LIST *szs_queue,TRACK_LIST *info_queue,TRACK_LIST *dl_queue,TRACK_LIST *final_track_list,unsigned int *on_hold,OVERRIDE_INFO *override,BOOL *force_decision){
	...
	while(!feof(in_list)){
		...
		//check if line is comment
			...
			//if no results, print on log
			if(search_status==NO_RESULTS||search_status==NO_100_RESULTS||search_status==NO_VALID_RESULTS){
...
void print_track_list(TRACK_LIST *list, BOOL print_dl){
	...
	while(cursor->next!=NULL){
		if(cursor->t.dl==TRUE) fprintf(log,"%s\n",cursor->t.name);
		cursor=cursor->next;
	}
	if(cursor->t.dl==TRUE) fprintf(log,"%s\n",cursor->t.name);
	fprintf(log,"\nAlready downloaded:\n");
	...
	while(cursor->next!=NULL){
		if(cursor->t.dl==FALSE) fprintf(log,"%s\n",cursor->t.name);
		cursor=cursor->next;
	}
	if(cursor->t.dl==FALSE) fprintf(log,"%s\n",cursor->t.name);
	fclose(log);
...
void convert_to_szs(char name[]){
	//definitely different on Windows
	printf("\n");
	char path_to_converter[200],command[500];
	FILE *file;
	//returns -1 if file doesn't exist
	if(access(PATH_TO_WBZ_CONVERTER,F_OK)==-1){
		printf("[!] path-to-wbz-converter.txt not found\nI won't convert into szs\n");
		return;
	}
	//read those paths
	file=fopen(PATH_TO_WBZ_CONVERTER,"r");
	fgets(path_to_converter,200,file);
	fclose(file);
	remove_line_feed(path_to_converter);
	if(path_to_converter[strlen(path_to_converter)-1]!='/') strcat(path_to_converter,"/");
	//sorta check if path to wbz converter is legit
	sprintf(command,"%sconvert-WBZ-to-SZS.sh",path_to_converter);
	if(access(PATH_TO_WBZ_CONVERTER,F_OK)==-1){
		printf("[!] path-to-wbz-converter.txt doesn't seem right\nI won't convert into szs\n");
		return;
	}
	printf("Converting %s.wbz into szs...\n",name);
	
	//save whatever happened to be in WBZ-to-SZS
	sprintf(command,"%sWBZ-to-SZS",path_to_converter);
	check_directory(command);
	sprintf(command,"mv \"%sWBZ-to-SZS/\" \"%s.wbz-to-szs/\"",path_to_converter,path_to_converter);
	system(command);
	sprintf(command,"%sWBZ-to-SZS",path_to_converter);
	check_directory(command);
	
	//copy wbz file into WBZ-to-SZS
	sprintf(command,"%s%s.szs",SZS_FILES_DIR,name);
	if(access(command,F_OK)==-1){
		sprintf(command,"cp \"%s%s.wbz\" \"%sWBZ-to-SZS\"",WBZ_FILES_DIR,name,path_to_converter);
		system(command);
		
		//convert it
		sprintf(command,"cd \"%s\";./convert-WBZ-to-SZS.sh",path_to_converter);
		system(command);
		
		//move newly created szs file
		sprintf(command,"mv \"%sWBZ-to-SZS/%s.szs\" \"%s\"",path_to_converter,name,SZS_FILES_DIR);
		system(command);
	}
	//final cleanup
	sprintf(command,"rm -rf \"%sWBZ-to-SZS/\"",path_to_converter);
	system(command);
	sprintf(command,"mv \"%s.wbz-to-szs/\" \"%sWBZ-to-SZS\"",path_to_converter,path_to_converter);
	system(command);
...
void print_unneeded_files(TRACK_LIST *list){
	...
	sprintf(command,"sort .gorbage >> %s; rm .gorbage",LOG_FILE);
	system(command);
